...
func (t *TidbClient) GetMultipart(bucketName, objectName, uploadId string) (multipart Multipart, err error) {
	...
	sqltext := "select bucketname,objectname,uploadtime,initiatorid,ownerid,contenttype,location,pool,
	acl,sserequest,encryption,attrs,storageclass from multiparts where bucketname=? and objectname=? and uploadtime=?;"
